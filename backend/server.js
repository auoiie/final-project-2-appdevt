...
const gameLoop = (lobbyCode) => {
  ...
  if (gameState.timer <= 0) {
    ...
    if (activePlayers.length <= 1) {
      io.to(lobbyCode).emit('game_over', { winner: activePlayers[0]?.username });
...
io.on('connection', (socket) => {
  ...
  socket.on('create_lobby', (username) => {
    ...
    lobbies[lobbyCode] = {
      players: [{ id: socket.id, username, ready: true, color: PLAYER_COLORS[0] }],
      host: socket.id,
      lastTagTime: 0,
      gameStarted: false,
    };
  ...
  socket.on('join_lobby', ({ lobbyCode, username }) => {
    ...
    if (lobby && !lobby.gameStarted && lobby.players.length < 4) {
      ...
      lobby.players.push({ id: socket.id, username, ready: false, color: availableColor });
  ...
  socket.on('start_game', (lobbyCode) => {
    if (lobbies[lobbyCode] && lobbies[lobbyCode].host === socket.id) {
      const allReady = lobbies[lobbyCode].players.every(p => p.ready);
      if (allReady) {
        const lobby = lobbies[lobbyCode];
  ...
  socket.on('leave_game', (lobbyCode) => {
    socket.leave(lobbyCode);
    const lobby = lobbies[lobbyCode];
    if (lobby) {
      const player = lobby.gameState.players.find(p => p.id === socket.id);
      if(player) player.disqualified = true;
    }
  });
